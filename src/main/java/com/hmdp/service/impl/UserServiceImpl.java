...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result signCount() {
        ...
        // 从redis的bitmap中取这个月截至今天为止的bit  BITFIELD sign:5:202203 GET u14 0
        List<Long> signList = stringRedisTemplate.opsForValue().bitField(
                key,
                BitFieldSubCommands.create()
                        .get(BitFieldSubCommands.BitFieldType.unsigned(dayOfMonth))//当前是这个月的第几天，就获取几位的bit
                        .valueAt(0));// 从第0位开始取
        // 判空
        if(signList == null || signList.isEmpty()){
            return Result.ok(0L);
        }
        // 已知redis命令只查询了1条所以是下标0
        Long signBit = signList.get(0);
